...
#* +------------------------------------------------------------------------+ *
#******************************************************************************
nl.hetroLS <-  function(formula, data, start=getInitial(formula,data), 
		control=nlr.control(tolerance=0.0010, minlanda=1 / 2 ^ 10, maxiter=25 * length(start)),
    varmodel,tau=getInitial(varmodel,vdata),... ){
  ...
  if(is.Fault(stage1))
    ...
    pd <- predict(stage1,newdata=fdata) 
		vdata[[varmodel$independent]] <- as.numeric(pd)                        # non replicated    #
	}
	else
  ...
  g <- ( 2.0 * as.numeric(varcomp$predictor)^2 / wi / stage1$parameters$sigma)
	vm <- diag(g)
	rm <- diag(1.0/sqrt(g))
  stage2 <- nlsqr.gn(varmodel, data=vdata, start=start.tau,control=control,vm=vm,rm=rm)
	if(is.Fault(stage2)){# || stage2$parameters$sg<0 || stage2$parameters$landa<0){
		print("error in first try stage2.. try Nelder MEad Algorithm")
		stage2 <- nlsnm(formula=varmodel,data=vdata,start=tau,control=control,vm=vm,rm=rm)

#		stage2 <- nlsqr.gn(varmodel, data=vdata, start=tau,tolerance=tolerance,vm=vm,rm=rm)
#		if(is.Fault(stage2))
#			stage2 <- nlsqr(varmodel, data=vdata, start=tau,control=control)
	...
	stage3 <- nlsqr.gn(formula, data=data, start=ps,control=control,vm=vmat,rm=rmat)
	if(is.Faultwarn(stage3))
			stage3 <- nlsqr.gn(formula, data=data, start=start,control=nlr.control(tolerance=tolerance*10, minlanda=minlanda, maxiter=maxiter),vm=vmat,rm=rmat)
	if(is.Fault(stage3)){
		print("Error in stage3, Nelder Mead is used")
		stage3 <- nlsnm(formula=formula,data=data,start=start,control=nlr.control(tolerance=tolerance*10, minlanda=minlanda, maxiter=maxiter),vm=vmat,rm=rmat)
	}
